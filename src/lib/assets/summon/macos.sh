...
function reverse1999_summon_log() {
  ...
  url=$(sqlite3 $file "SELECT request_key FROM cfurl_cache_response WHERE request_key GLOB 'https://game-re-service.sl916.com/query/summon?userId=*' ORDER BY time_stamp DESC LIMIT 1;")
